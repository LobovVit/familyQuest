...
Новые математические занятия сохраняют `rewardVersion=2`; отсутствие поля означает старую шкалу. `AnswerMath` под блокировкой строки участника считает математические звёзды за дату Минска и ограничивает новую награду остатком бюджета 30 ⭐. Ответ и фактически выданная награда фиксируются одной транзакцией; нулевой бонус не делает правильный ответ ошибочным. Backup v4 валидирует версию, допустимые награды и общий бюджет новых занятий; v3 по-прежнему читается.

...
По умолчанию сохраняется домашний режим. Для закрытого пилота реализован опциональный SaaS: отдельная SQL-схема и роль каждой семьи, взрослый аккаунт, возрастные политики и учёт подписок. Подробности и ограничения — в [SaaS-архитектуре](saas-architecture.md), запуск — в [инструкции оператора](saas-operations.md). [Первичный отчёт](review-2026-09-26.md) сохранён как история проверки до реализации.
...
дела и поведение. Архивирование не удаляет заработанное. Backup v6 сохраняет и
проверяет журнал вместе с занятиями; восстановление проверяет математические
ответы и соответствие их начислений.

Новые математические занятия сохраняют `rewardVersion=3` (+1 ⭐ к версии 2, до 40 ⭐ в день); отсутствие поля означает старую шкалу, версия 2 сохраняет лимит 30 ⭐. SQL-адаптер `AnswerMath` под блокировкой участника вычисляет дневной итог за дату Минска. Ответ и фактически выданная награда атомарны. Backup v6 валидирует версии и дневные бюджеты; версии v1–v5 остаются читаемыми в домашнем режиме.

Чтение использует `ReadingCompletion` и тот же журнал `activity_rewards`, с временной таблицей `reading_starts` для фиксации разрешённого уровня в SaaS. Это самостоятельная отметка о завершении шести карточек: сервер доверяет факту практики, но сам определяет участника, дату и 6/9/12 ⭐ по уровню. Лимит чтения 30 ⭐ в день независим от математики. `CompleteReading` блокирует участника, проверяет повтор UUID до расчёта лимита и атомарно сохраняет награду, включая нулевую. Один UUID с разными уровнями даёт конфликт. Миграция `007_reading_rewards.sql` расширяет допустимые источники; backup v6 сохраняет журнал и проверяет награды чтения.
...
проверяют cookie. Возврат к вкладке также проверяет сессию. JSON-backup v6 не содержит

...
it('sends column carries and rejects missing results', async () => {
 ...
 fireEvent.click(screen.getByRole('button', { name: 'Перенос единиц' }))
 fireEvent.click(screen.getByRole('button', { name: '5' }))
 fireEvent.focus(screen.getByLabelText('Ответ, разряд 2'))
 fireEvent.click(screen.getByRole('button', { name: 'Перенос десятков' }))
 fireEvent.click(screen.getByRole('button', { name: '2' }))
 fireEvent.focus(screen.getByLabelText('Ответ, разряд 3'))

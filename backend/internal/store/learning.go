...
func (s *Store) AnswerMath(ctx context.Context, owner int64, id string, index int, values map[string]int, now time.Time) (domain.MathSession, error) {
	...
	if len(session.Answers) > count {
		var earned int
		if err = tx.QueryRow(ctx, `select coalesce(sum(stars),0)::int from activity_rewards where participant_id=$1 and source='math' and earned_date=$2::date`, owner, session.Answers[index].Date).Scan(&earned); err != nil {
			return session, err
		}
		if reward := session.AwardAnswer(index, earned); reward != nil {
